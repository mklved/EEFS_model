...
class v_to_v(nn.Module):
    def __init__(self,feature):
        super().__init__()
        self.conv1 = nn.Conv1d(feature,feature,kernel_size=1)
        self.bn = nn.BatchNorm1d(feature)
        self.gl = nn.GELU()
        self.conv2 = nn.Conv1d(feature,feature,kernel_size=1)
...
class Attention(nn.Module):
    def __init__(self, dim, window_size=None, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.qkv = nn.Conv2d(dim, dim * 3, 1,bias=qkv_bias)
        self.qkv_2 = nn.Conv2d(dim,dim // 2,kernel_size=3,stride=1,padding=1,bias=qkv_bias)
    ...
    def forward(self, x):
        ...
        q, k, v = self.qkv(x).reshape(B, self.num_heads, C // self.num_heads *3, N).chunk(3, dim=2) # (B, num_heads, head_dim, N)
        agent_tokens= self.qkv_2(x).reshape(B, self.num_heads, C // self.num_heads // 2, N) # (B, num_heads, head_dim, N)
...
class StokenAttention(nn.Module):
    def __init__(self, dim, stoken_size, n_iter=1, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.stoken_refine_glo = AgentAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=proj_drop)
    ...
    def direct_forward(self, x,short):
        B, C, H, W = x.shape
        stoken_features = x
        stoken_features = self.stoken_refine(stoken_features)        
    ...
    def forward(self, x,short):
        if self.stoken_size[0] > 1 or self.stoken_size[1] > 1:
            ...
            return self.direct_forward(x,short)
        # return self.direct_forward(x,short)
...
class StokenAttentionLayer(nn.Module):
    def __init__(self, dim, n_iter, stoken_size, 
                 ...
                 drop_path=0., act_layer=nn.GELU, layerscale=False, init_values = 1.0e-5,rep = False):
        super().__init__()
    ...
    def forward(self, x,short):
        x = self.pos_embed(x)
        if self.layerscale:
            x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x),short))
            x = x + self.drop_path(self.gamma_2 * self.mlp2(self.norm2(x))) 
        else:
            x = x + self.drop_path(self.attn(self.norm1(x),short))
...
#             nn.Conv2d(in_channels, out_channels // 2, kernel_size=(3, 3), stride=(1, 2), padding=(1, 1)),
#             nn.GELU(),
